...
class CleanUp(GroupCog, name="cleanup") :
	...
	@app_commands.command(name="inactivity",
	                      description=f"Toggle the removal of threads after a x amount of days of inactivity per forum. Disabled by default")
	@app_commands.choices(operation=OPERATION_CHOICES)
	@app_commands.checks.has_permissions(manage_channels=True)
	@AccessControl().check_premium()
	async def old(self, interaction: discord.Interaction, operation: Choice['str'], days: int = 0) :
		...
		match operation.value.lower() :
			case "add" :
				...
				await send_response(interaction,
				                    f"Posts older than {days} days with no activity in {', '.join([forum.mention for forum in success])} will be automatically removed.",
				                    ephemeral=True)
			...
			case "remove" :
				...
				await send_response(interaction,
				                    f"Removed the inactivity cleanup from {', '.join([forum.mention for forum in success])}.",
				                    ephemeral=True)
				return None
			case "list" :
				forums = ForumTransactions().get_all(interaction.guild.id)
				formatted_message = f"Forum's with inactive post cleanup:\n"
				for forum in forums :
					result = ForumCleanupTransactions().get(forum.id, _key)
					if not result :
						continue
